...
use candle_transformers::{
    generation::LogitsProcessor, models::quantized_llama::ModelWeights as M,
};

use text_yoga_ai::quantized_llama::ModelWeights;
...
impl Model {
    fn process(&mut self, tokens: &[u32]) -> candle::Result<String> {
        ...
        let text = match self.tokenizer.id_to_token(next_token) {
...
impl Model {
    ...
    pub fn new(weights: Vec<u8>, tokenizer: Vec<u8>) -> Result<Model, JsError> {
        ...
        // }
        let seed = 299792458;
        let temperature: Option<f64> = Some(0.0);
        ...
        let tokenizer = Tokenizer::from_bytes(&tokenizer)
            .map_err(|msg| JsError::new("Failed to load tokenizer."))?;
